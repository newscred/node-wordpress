...
function Client( settings ) {
	...
	this.rpc = xmlrpc[ parsedUrl.secure ? "createSecureClient" : "createClient" ]({
		...
		path: parsedUrl.path,
		basic_auth: {
			user: settings.httpAuthUser,
			pass: settings.httpAuthPassword
		}
	});
...
extend( Client.prototype, {
	...
	credentialsCall: function() {
		var args = [].slice.call( arguments );
		args.splice( 1, 0, this.username, this.password );
		this.call.apply( this, args );
	},
...
extend( Client.prototype, {
	...
	uploadFile: function ( data, fn ) {
		this.authenticatedCall( "wp.uploadFile", fieldMap.to( data, "uploadFile" ), fn );
	},
	...
	editImageMeta: function ( id, data, fn ) {
	    this.authenticatedCall( "nc.editImageMeta", id, fieldMap.to( data, "editImageMeta" ), fn );
	},

	clonePost: function ( id, fn ) {
	    this.authenticatedCall( "nc.clonePost", id, fn );
	}
});
